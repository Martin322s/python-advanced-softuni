...
class SummitQuestManagerApp:
    ...
    def check_gear(self, climber_name, peak_name, gear):
        ...
        if set(gear) >= set(recommended):
            return f"{climber_name} is prepared to climb {peak_name}."

        climber.is_prepared = False
        missing = sorted([item for item in recommended if item not in gear])
        return f"{climber_name} is not prepared to climb {peak_name}. Missing gear: {', '.join(missing)}."
    ...
    def get_statistics(self):
        ...
        return (
            f"Total climbed peaks: {total_peaks_climbed}\n"
